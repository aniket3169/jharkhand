...
export function checkOrigin(request) {
  ...
  if (origin) {
    ...
    const reqHost = (request.headers.get("x-forwarded-host") || request.headers.get("host") || new URL(request.url).host).split(":")[0];
    const isLocal = (h) => ["localhost", "127.0.0.1", "0.0.0.0", "::1"].includes(h);
    if (originHost !== reqHost && !(isLocal(originHost) && isLocal(reqHost))) {
