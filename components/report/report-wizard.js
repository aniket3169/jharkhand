...
export default function ReportWizard() {
  ...
  useEffect(() => {
    if (step !== 1 || evidence.length === 0 || evidence.length === lastAnalyzedEvidenceCount.current) return;
    let active = true;
    setAnalyzingImage(true);
    setError('');
    api('/api/problems/analyze', context())
      .then(result => {
        if (!active) return;
        applyAnalysis(result);
        lastAnalyzedEvidenceCount.current = evidence.length;
      })
      .catch(err => {
        if (!active) return;
      ...
      .finally(() => {
        if (active) setAnalyzingImage(false);
      });
    return () => { active = false; };
  ...
  async function beginInterview() {
    ...
    try {
      let result = (analysis && analysis.followUpQuestions?.length > 0) ? { analysis, questions: analysis.followUpQuestions } : null;
      if (!result) {
        result = await api('/api/problems/analyze', context());
        applyAnalysis(result);
      }
      const generated = result.analysis || result;
  ...
  async function nextStep() {
    ...
    if (step === 1) {
      if (analyzingImage) {
        setError('Please wait while AI inspects your uploaded evidence.');
        return;
      }
      if (error) {
        return;
      }
      if (evidence.length > 0 && !analysis) {
        setBusy('analysis'); setError('');
        try {
    ...
    return (
      <div className="report-page">
        ...
        <header className="report-page-header">
          <div>
            <div className="report-eyebrow"><span /> SMALL ACTIONS. MEANINGFUL CHANGE.</div>
            <h1>Every solution starts with you.</h1>
            <p>Tell us what your community needs. Together, we’ll find a way forward.</p>
          </div>
        </header>
        <div className="report-card" style={{ padding: '60px 30px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <ShieldCheck size={48} color="#087f70" style={{ marginBottom: 20 }} />
          <h2 style={{ fontSize: 24, marginBottom: 10, color: '#274034' }}>Sign in to report a problem</h2>
          <p style={{ color: '#8b978e', marginBottom: 30, maxWidth: 450, lineHeight: 1.6 }}>You must be signed in to submit a challenge. This helps us keep you updated on the progress and ensures all reports are from verified members of the community.</p>
          <div style={{ width: '100%', maxWidth: 450, textAlign: 'left' }}>
            <LoginCard />
          </div>
        </div>
      </div>
