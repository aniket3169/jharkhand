...
export default function ExploreChallenges() {
  ...
  return <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="challenge-page">
    <div className="challenge-page-title"><div><div className="challenge-eyebrow">SMALL ACTIONS. SHARED PROGRESS.</div><h1>Discover a challenge.<br className="challenge-mobile-break" /> Make a difference.</h1><p>Explore local challenges and find where your expertise can help.</p></div>{(!user || user.role === 'citizen') && <Link className="btn btn-primary" href="/report">Report a problem <ArrowUpRight size={17} /></Link>}</div>
